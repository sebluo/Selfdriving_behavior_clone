...
def build_model2(model):    #input shape (32,64,3)
    ...
    model.add(Conv2D(32,3,3, activation = 'elu', border_mode='valid',W_regularizer=l2(reg_val),name='Conv2D1')) #(30,62,32,3)
...
import os
if os.path.exists("./weights9.h5"):
    model.load_weights("./weights9.h5")
    print("load the previous weights9.h5 successfully")
...
for j in range(numTimes):
    model.load_weights("./weights"+str(j)+".h5")
